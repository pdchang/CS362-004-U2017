...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
      call_adventurer(state, handPos);
      return 0;

    case council_room:
      call_council_room(state, handPos);
      return 0;
      ...
      for (i = 0; i < state->handCount[currentPlayer]; i++)
  {
    ...
    case smithy:
      call_smithy(state, handPos);
      return 0;

    case village:
      call_village(state, handPos);
      return 0;
...
// Card Refactoring Assignment 2
void call_adventurer(struct gameState *state, int handPos) {
  state->numActions--;
  int cardDrawn;
  int z = 0;
  int currentPlayer = whoseTurn(state);
...
void call_council_room(struct gameState *state, int handPos) {
  state->numActions--;
  int currentPlayer = whoseTurn(state);
  int i;
...
void call_smithy(struct gameState *state, int handPos) {
  state->numActions--;
  int currentPlayer = whoseTurn(state);
  int i;
  //+3 Cards
...
void call_village(struct gameState *state, int handPos) {
  state->numActions--;
  int currentPlayer = whoseTurn(state);
  //+1 Card
  drawCard(currentPlayer, state);
...
void call_great_hall(struct gameState *state, int handPos) {
  state->numActions--;
  int currentPlayer = whoseTurn(state);
//+1 Card
  drawCard(currentPlayer, state);
